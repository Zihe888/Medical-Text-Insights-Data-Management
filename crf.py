...
class CRF(nn.Module):
    ...
    def decode(self, emissions: torch.Tensor,
        ...
        if nbest == 1:
            return self._viterbi_decode(emissions, mask, pad_tag).unsqueeze(0)# .unsqueeze(0)表示添加一个维度
        return self._viterbi_decode_nbest(emissions, mask, nbest, pad_tag)
    '''
    def decode_prob(self, emissions: torch.Tensor,
               mask: Optional[torch.ByteTensor] = None,
               nbest: Optional[int] = None,
               pad_tag: Optional[int] = None) -> List[List[List[int]]]:
        """Find the most likely tag sequence using Viterbi algorithm.
        ...
        if nbest == 1:
            return self._viterbi_decode_prob(emissions, mask, pad_tag).unsqueeze(0)# .unsqueeze(0)表示添加一个维度
        # return self._viterbi_decode_nbest(emissions, mask, nbest, pad_tag)
    '''
    ...
    def _compute_normalizer(self, emissions: torch.Tensor,
        ...
        for i in range(1, seq_length):
            ...
            # 这里改啦
            # score = score.masked_scatter(mask[i].unsqueeze(1), next_score[mask[i].unsqueeze(1)])
    ...
    '''
    def _viterbi_decode_prob(self, emissions: torch.FloatTensor,
                        mask: torch.ByteTensor,
                        pad_tag: Optional[int] = None) -> List[List[int]]:
        # emissions: (seq_length, batch_size, num_tags)
        # mask: (seq_length, batch_size)
        ...
        history_idx.scatter_(1, seq_ends.view(-1, 1, 1).expand(-1, 1, self.num_tags),
                             end_tag.view(-1, 1, 1).expand(-1, 1, self.num_tags))
                             #end_score.view(-1, 1, 1).expand(-1, 1, self.num_tags))

        print(history_idx)
        print(history_idx.size())
        #(seq_length, batch_size, num_tags)
        history_idx = history_idx.transpose(1, 0).contiguous()  #用来转置
